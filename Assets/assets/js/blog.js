...
function renderBlogPosts() {
  ...
  // First, show shimmer loading state
  showShimmerLoading(blogContainer);
  
  // Simulate loading delay (remove this in production if you don't need it)
  setTimeout(() => {
    // Clear shimmer and render actual content
    blogContainer.innerHTML = '';
    
    // Rest of your existing renderBlogPosts code...
        ...
        <div class="row mt-5 featured-articles-grid">
          ${post.featuredArticles.map((article, idx) => `
            <div class="col-12 col-md-6 mb-4">
              <div class="custom-blog-card" onclick="window.location.href='article.html?id=${article.id}'">
...
function showShimmerLoading(container) {
  container.innerHTML = `
    <!-- Main Post Shimmer -->
    <div class="main-blog-post shimmer-card">
    ...
    <!-- Featured Articles Shimmer -->
    <div class="row mt-5 featured-articles-grid">
      ${Array(4).fill().map(() => `
        <div class="col-12 col-md-6 mb-4">
          <div class="custom-blog-card shimmer-card">
            <div class="shimmer shimmer-image"></div>
            <div class="custom-blog-card__content">
              <div class="shimmer shimmer-title"></div>
              <div class="shimmer shimmer-text"></div>
    ...
    <!-- Pagination Shimmer -->
    <div class="blog-pagination-container mt-4">
      <div class="pagination-wrapper">
        ${Array(3).fill().map(() => `
          <div class="shimmer shimmer-badge" style="width: 40px; height: 40px; border-radius: 50%;"></div>
        `).join('')}
      </div>
    </div>
  `;
...
function renderCategories() {
  ...
  categoriesContainer.innerHTML = Array(5).fill().map(() => `
    <div class="category-card shimmer-card">
      <div class="shimmer shimmer-badge" style="width: 40px; height: 40px; border-radius: 50%;"></div>
      <div class="category-content">
        <div class="shimmer shimmer-title" style="width: 70%;"></div>
        <div class="shimmer shimmer-text" style="width: 40%;"></div>
      </div>
    </div>
  `).join('');

  // Load actual categories after delay
  setTimeout(() => {
            ...
            <div class="category-post-card">
              ...
              <div class="category-post-content">
                <div class="category-post-header">
                  <span class="category-post-date">${post.date}</span>
                </div>
...
categoryStyle.textContent = `
  ...
  .category-header .d-flex {
    min-height: 60px; /* Adjust this value based on your title height */
  }

  .clear-filter-btn {
    height: fit-content;
    margin-bottom: 1rem; /* Matches the title's bottom padding */
  }
  ...
  .category-badge-fixed {
    ...
    top: 15px;
    left: 15px;
    background-color: #7209d494 !important; /* Updated to specific color with opacity */
    color: white;
    padding: 0.25rem 0.75rem;
    border-radius: 20px;
    font-size: 0.8rem;
  ...
  /* Dark mode adjustment for the fixed badge */
  body.dark-mode .category-badge-fixed {
    background-color: #7209d494 !important; /* Keep the same color in dark mode */
    color: white;
  }

  /* Remove the old hover-dependent category badge if it exists */
  .category-post-card:hover .category-badge {
    display: none;
  }

  /* Update the category-post-image to accommodate the fixed badge */
  .category-post-image {
    position: relative;
    height: 200px;
    overflow: hidden;
  ...
  /* Ensure the overlay doesn't cover the badge */
  .category-post-overlay {
    z-index: 1;
  }

  /* Shimmer-specific adjustments */
  .shimmer-card {
    background: #fff;
    padding: 15px;
    margin-bottom: 20px;
  ...
  .custom-blog-card.shimmer-card {
    height: 100%;
    display: flex;
    flex-direction: column;
  }

  .custom-blog-card.shimmer-card .shimmer-image {
    height: 200px;
    border-radius: 12px 12px 0 0;
  }

  .custom-blog-card.shimmer-card .custom-blog-card__content {
    padding: 1.5rem;
    flex-grow: 1;
  }
`;
